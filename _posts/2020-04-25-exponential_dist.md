...
이번 포스트에서는 지수 분포(Exponential Distribution)에 대해서 정리하고자 합니다. 기대값, 분산을 구해보고 포아송 분포와의 관계, 무기억성에 대해서 알아보겠습니다.
...
어떤 사건 $A$의 발생 횟수가 발생률(평균 발생 건수)이 $\lambda$인 포아송 분포를 따를 때, $A$가 일어나고 그 다음에 또 일어날 때까지 걸리는 시간을 $W$라고 한다면, $W$는 음이 아닌 값을 가지는 연속형 확률변수가 될 것입니다. 이 때, $W$의 cdf는 다음과 같이 표현될 것입니다.
