...
* Step2. 나머지 값은 모두 고정한 채, X의 모든 값을 '보수'로 바꿔서 각 샘플에 대한 예측값($\hat \textbf y_1$)들을 구한다.  
* Step3. 나머지 값은 모두 고정한 채, X의 모든 값을 '진보'로 바꿔서 각 샘플에 대한 예측값($\hat\textbf y_2$)들을 구한다.  
* Step4. 나머지 값은 모두 고정한 채, X의 모든 값을 '중립'으로 바꿔서 각 샘플에 대한 예측값($\hat\textbf y_3$)들을 구한다.
