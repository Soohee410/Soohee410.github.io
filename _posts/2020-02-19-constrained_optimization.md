...
date:   2020-02-19
image: '/assets/img/linear_main10.png'
tags: [Linear Algebra]
use_math: true
---
...
$$ \min_{ \lVert\boldsymbol x\rVert=1} \boldsymbol x^{\intercal}A\boldsymbol x =\space ? \quad or\quad \max_{ \lVert\boldsymbol x\rVert=1} \boldsymbol x^{\intercal}A\boldsymbol x =\space ? $$
...
이 때, $\small \boldsymbol x^{\intercal}A\boldsymbol x$의 최댓값은 행렬 $A$의 고유값 중 가장 큰 것($\lambda_1$)이고, 이를 최대화하는 $\boldsymbol x$는 $\lambda_1$에 해당하는 단위 고유벡터 $\boldsymbol u_1$이다. 한편, $\small \boldsymbol x^{\intercal}A\boldsymbol x$의 최솟값은 $A$의 고유값 중 가장 작은 것($\lambda_n$)이고, 이를 최소화하는 $\boldsymbol x$는 $\lambda_n$에 해당하는 단위 고유벡터 $\boldsymbol u_n$이다.
 ...
 $$ \boldsymbol x^{\intercal}A\boldsymbol x = \boldsymbol y^{\intercal}D\boldsymbol y \quad when\space \boldsymbol x=P\boldsymbol y$$
...
$$\lVert \boldsymbol x\rVert=\small \lVert P\boldsymbol y\rVert=\small \lVert \boldsymbol y\rVert$$

특히, $\boldsymbol x$가 단위벡터라는 제한조건이 존재하기 때문에, 둘다 길이가 1이겠네요!

$$\lVert \boldsymbol x\rVert=\small \lVert \boldsymbol y\rVert=1$$
...
$$ \max_{ \lVert\boldsymbol x\rVert=1} \boldsymbol x^{\intercal}A\boldsymbol x =\max_{ \lVert\boldsymbol y\rVert=1} \boldsymbol y^{\intercal}D\boldsymbol y, \quad \min_{\lVert\boldsymbol x\rVert=1} \boldsymbol x^{\intercal}A\boldsymbol x =\min_{ \lVert\boldsymbol y\rVert=1} \boldsymbol y^{\intercal}D\boldsymbol y$$

이 때, $\small\boldsymbol y^{\intercal}D\boldsymbol y$의 하한선은 다음과 같이 $\lambda_n$이 될 것입니다.

$$\begin{aligned}\boldsymbol y^{\intercal}D\boldsymbol y&=\lambda_1y_1^2+\lambda_2y_2^2+\cdots+\lambda_ny_n^2\cr &\ge\lambda_ny_1^2+\lambda_ny_2^2+\cdots+\lambda_ny_n^2\cr
...
$$\begin{aligned}\boldsymbol y^{\intercal}D\boldsymbol y&=\lambda_1y_1^2+\lambda_2y_2^2+\cdots+\lambda_ny_n^2\cr &\le\lambda_1y_1^2+\lambda_1y_2^2+\cdots+\lambda_1y_n^2\cr
...
$$\boldsymbol x=P\boldsymbol e_n=\begin{bmatrix}\boldsymbol u_1 & \boldsymbol u_2 &\cdots&\boldsymbol u_n \end{bmatrix}\begin{pmatrix} 0\cr\vdots\cr 1\end{pmatrix}=\boldsymbol u_n$$
...
$$\boldsymbol x=P\boldsymbol e_1=\begin{bmatrix}\boldsymbol u_1 & \boldsymbol u_2 &\cdots&\boldsymbol u_n \end{bmatrix}\begin{pmatrix} 1\cr\vdots\cr 0\end{pmatrix}=\boldsymbol u_1$$

따라서 정리하면 다음과 같은 결론이 도출됩니다.

$$\begin{aligned} \max_{ \lVert\boldsymbol x\rVert=1} \boldsymbol x^{\intercal}A\boldsymbol x=\max_{ \lVert\boldsymbol y\rVert=1} \boldsymbol y^{\intercal}D\boldsymbol y=\lambda_1, \space when \space \boldsymbol x=\boldsymbol{u_1}, \boldsymbol y=\boldsymbol{e_1}
...
$$\max_{ \lVert\boldsymbol x\rVert=1} \boldsymbol x^{\intercal}A\boldsymbol x =\max_{\boldsymbol x\not =0} \frac {\boldsymbol x^{\intercal}A\boldsymbol x}{\boldsymbol x^{\intercal} \boldsymbol x}$$
...
$$\big(\frac{\boldsymbol x}{\lVert \boldsymbol x \rVert}\big)^{\intercal}A\big(\frac{\boldsymbol x}{\lVert \boldsymbol x \rVert}\big)=\frac{\boldsymbol x^{\intercal} A \boldsymbol x}{\lVert \boldsymbol x \rVert ^2}=\frac{\boldsymbol x^{\intercal} A \boldsymbol x}{\boldsymbol x^{\intercal}\boldsymbol x} $$

<br>이제 예제를 하나 보겠습니다.

$$ maximize\quad \frac{x_1^2-8x_1x_2+x_2^2}{x_1^2+x_2^2}
...
$$\begin{gathered} maximize\quad 5x_1^2+6x_2^2+7x_3^2+4x_1x_2-4x_2x_3\cr when\space \boldsymbol x^{\intercal}\boldsymbol x=1, \boldsymbol x \perp \boldsymbol{u_1}\end{gathered}$$
...
$$\begin{aligned}\boldsymbol x = P\boldsymbol y&=\begin{bmatrix}u_{11} &\cdots & u_{n1}\cr \vdots&\ddots&\vdots\cr u_{1n}&\cdots&u_{nn }\end{bmatrix}\begin{bmatrix}y_1\cr \vdots\cr y_n
...
$$ \begin{aligned}\boldsymbol u_1^{\intercal} \boldsymbol{x} &=y_1\boldsymbol{u_1^{\intercal}} \boldsymbol{u_1}+\cdots+y_n\boldsymbol{u_1^{\intercal}} \boldsymbol{u_n}\cr
...
$$\begin{aligned}\boldsymbol y^{\intercal}D\boldsymbol y&=\lambda_1y_1^2+\lambda_2y_2^2+\cdots+\lambda_ny_n^2\cr
...
$$l\boldsymbol x=P\boldsymbol e_2=\begin{bmatrix}\boldsymbol u_1 & \boldsymbol u_2 &\cdots&\boldsymbol u_n \end{bmatrix}\begin{pmatrix} 0\cr 1\cr\vdots\cr 0\end{pmatrix}=\boldsymbol u_2$$
...
$$ \begin{aligned} Q(\boldsymbol x) &= 5x_1^2+6x_2^2+7x_3^2+4x_1x_2
