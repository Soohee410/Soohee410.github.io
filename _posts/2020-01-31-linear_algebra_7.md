...
>	어떤 벡터공간 $V$에 대한 **내적(inner product on $V$)** 은 $V$의 각 벡터 쌍 $u, v$에서 실수 $<u, v>$로 가는 함수로서, 모든 $u, v, w \in V, c\in \mathbb R$에 대해 다음의 공리(axiom)들을 만족한다.
	1. $<u, v> = <v, u>$
	2. $<u+v, w> = <u,w> + <v,w> $
	3. $<cu, v> = c<u, v>$
	4. $<u, u>\space \ge 0\space and\space <u,u>=0 \iff u=0$
내적이 존재하는 벡터 공간을 **내적 공간(inner product space)** 이라고 부른다.
