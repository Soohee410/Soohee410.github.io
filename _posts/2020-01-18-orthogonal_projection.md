...
$$\small \hat y=\frac {y\cdot u}{u\cdot u} u=proj_{\scriptsize W} y,\quad   W=span\{ u \} $$
...
$$\small  y=\frac{y\cdot u_1}{u_1\cdot u_1} u_1+\frac {y\cdot u_2}{u_2\cdot u_2} u_2 $$
...
$$\small y=proj_{span\{ u_1\}}y+proj_{span\{ u_2\}} y$$
...
$$\small proj_{\scriptsize W} y =(u_1\cdot y)u_1+(u_2\cdot y)u_2+\cdots+(u_p\cdot y)u_p$$
...
$$\small y=(u_1\cdot y)u_1+(u_2\cdot y)u_2+\cdots+(u_n\cdot y)u_n$$

한편, 부분공간 $W$에 대한 벡터 $y$의 정사영$(\hat y)$은 다음과 같이 표현됩니다.

$$\small \hat y=(u_1\cdot y)u_1+(u_2\cdot y)u_2+\cdots+(u_p\cdot y)u_p$$

**Orthogonal Decomposition Theorem** 에 의해, $y$는 다음과 같이 쪼개집니다.

$$\small y=\hat y + z, \quad z=y-\hat y$$

이 때, $z$는 $y$에서 $\hat y$를 뺀 부분이며, $W$에 직교하는 벡터입니다.

$$\small z=(u_{p+1}\cdot y)u_{p+1}+\cdots+(u_n\cdot y)u_n\space \perp\space W$$
