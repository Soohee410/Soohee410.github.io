...
$$ \hat y=\frac {y\cdot u}{u\cdot u} u=proj_W y,\quad   W=span\{ u \} $$
...
한편, 부분공간 W에 대한 벡터 $y$의 정사영$(\hat y)$은 다음과 같이 표현됩니다.

$$\hat y=(u_1\cdot y)u_1+(u_2\cdot y)u_2+\cdots+(u_p\cdot y)u_p$$

Orthogonal Decomposition Theorem에 의해, $y$는 다음과 같이 쪼개집니다.
